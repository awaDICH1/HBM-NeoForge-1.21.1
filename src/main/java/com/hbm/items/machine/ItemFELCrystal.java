 ...
 * FEL 激光晶体（P5.2 迁移版）。
 *
 * 迁移自 1.12.2 com.hbm.items.machine.ItemFELCrystal（55 行）。
 * 1.21.1 变更：构造器 (EnumWavelengths, Properties)；setMaxStackSize/setTranslationKey/setRegistryName/
 * ALL_ITEMS 删除（注册/名称由 DeferredRegister 处理）；addInformation → appendHoverText；
 * getUnlocalizedNameInefficiently → getDescriptionId；list.add(String) → tooltip.add(Component.literal)。
 */
public class ItemFELCrystal extends Item {

	public EnumWavelengths wavelength = EnumWavelengths.NULL;

	public ItemFELCrystal(EnumWavelengths wavelength, Item.Properties properties) {
		super(properties);
	...
	public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
		...
		tooltip.add(Component.literal(I18nUtil.resolveKey(desc)));
		tooltip.add(Component.literal(wavelength.textColor + I18nUtil.resolveKey(wavelength.name) + " - " + wavelength.textColor + I18nUtil.resolveKey(this.wavelength.wavelengthRange)));
	}
