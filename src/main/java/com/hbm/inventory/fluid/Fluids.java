...
public class Fluids {
    ...
    public static void init() {
        ...
        ESTRADIOL.addTraits(new FT_Toxin().addEntry(new ToxinEffects(HazardClass.PARTICLE_FINE, false).add(new MobEffectInstance(BuiltInRegistries.MOB_EFFECT.wrapAsHolder(HbmPotion.death), 60 * 60 * 20, 0))));
        REDMUD.addTraits(new FT_Toxin().addEntry(new ToxinEffects(HazardClass.GAS_BLISTERING, false).add(new MobEffectInstance(MobEffects.WITHER, 30 * 20, 2))));
        ...
        if(!config.exists()) {
            try { writeDefaultTraits(template); }
            catch(Exception ex) { System.err.println("Skipping fluid traits template generation: " + ex.getMessage()); }
        } else {
            readTraits(config);
        }
    }
    ...
    public static void reloadFluids(){
        ...
        File config = new File(FMLPaths.CONFIGDIR.get().resolve("hbmConfig").toFile().getAbsolutePath() + File.separatorChar + "hbmFluidTraits.json");
        File template = new File(FMLPaths.CONFIGDIR.get().resolve("hbmConfig").toFile().getAbsolutePath() + File.separatorChar + "_hbmFluidTraits.json");
    ...
    public static void writeType(CompoundTag nbt, String key, FluidType type) {
        nbt.putString(key, toNameCompat(type));
    }


    public static FluidType readType(CompoundTag nbt, String key) {
        if (nbt.contains(key, 8)) return fromNameCompat(nbt.getString(key)); // 8 = NBT string tag (current + legacy name saves)
        return fromID(nbt.getInt(key)); // legacy numeric-ID saves
    }
    ...
    /*
     * P4.1 批次A: 1.12 Forge 流体兼容层（initForgeFluidCompat/setupForgeFluidCompat/alreadyRegistered/registerForgeFluidCompat，
     * 基于 FluidRegistry/FluidNTM/FMLCommonHandler/IResource）已删除——1.21 注册分离，
     * NeoForge Fluid/FluidType 注册由 com.hbm.blocks.fluid.ModFluids 桥接（读取本类 metaOrder）承担。
     */
